...
def apply_multiple_filters(config: dict):
    ...
    try:
        with rio.open(config['ndwi_path']) as ndwi_src, \
            ...
            ndwi, ndvi, evi, STR = ndwi_src.read(), ndvi_src.read(), evi_src.read(), str_src.read()
            str_meta = str_src.meta.copy()
            if STR.ndim == 3:
                nb = STR.shape[0]
                if ndwi.ndim == 2: ndwi = np.tile(ndwi, (nb, 1, 1))
                if ndvi.ndim == 2: ndvi = np.tile(ndvi, (nb, 1, 1))
                if evi.ndim == 2: evi = np.tile(evi, (nb, 1, 1))
            else: raise ValueError("Input rasters must be multi-band [T,H,W].")
            ...
            STR = STR.astype(np.float32)
            STR[~mask] = np.nan
            STR[STR > config['str_max_threshold']] = np.nan

            with rio.open(config['vi_path']) as vi_src:
                VI = vi_src.read().astype(np.float32)
                if VI.ndim == 2: VI = np.tile(VI, (STR.shape[0], 1, 1))
                VI[np.isnan(STR)] = np.nan
            
            out_meta = str_meta.copy(); out_meta.update(dtype='float32', nodata=-9999)
...
def filter_points_by_adaptive_density(df: pd.DataFrame, vi: str, str_col: str,
    ...
    """
    df_interval = df[(df[vi] >= vi_min) & (df[vi] <= vi_max)]
    if df_interval.empty: return pd.DataFrame(columns=df.columns)
    ...
    log.debug(f"Using adaptive number of bins: {num_bins}")
    
    density_grid, x_edges, y_edges = np.histogram2d(x, y, bins=num_bins, density=True)
    
    sorted_densities = np.sort(density_grid.flatten())[::-1]
    cumulative_mass = np.cumsum(sorted_densities)
    total_mass = np.sum(sorted_densities)
    mass_threshold_index = np.searchsorted(cumulative_mass, total_mass * (config['density_core_mass'] / 100.0))
    if mass_threshold_index >= len(sorted_densities): mass_threshold_index = len(sorted_densities) - 1
    adaptive_threshold = sorted_densities[mass_threshold_index]
    log.debug(f"Adaptive density threshold calculated: {adaptive_threshold:.6f}")

    x_c, y_c = 0.5 * (x_edges[1:] + x_edges[:-1]), 0.5 * (y_edges[1:] + y_edges[:-1])
    pts = np.vstack([x, y]).T
    try:
        point_densities = interpn((x_c, y_c), density_grid, pts, method=config['interpn_method'], bounds_error=False, fill_value=0.0)
    except (ValueError, IndexError):
        log.warning(f"Interpolation with '{config['interpn_method']}' failed. Falling back to 'linear'.")
        point_densities = interpn((x_c, y_c), density_grid, pts, method='linear', bounds_error=False, fill_value=0.0)

    return df_interval[point_densities >= adaptive_threshold]
...
def density_scatter(x: np.ndarray, y: np.ndarray, config: dict, **kwargs) -> None:
    ...
    norm = mcolors.LogNorm(vmin=max(z.min(), 1e-4), vmax=min(z.max(), 1.0))
    sc = ax.scatter(x, y, marker=".", s=0.5, c=z, cmap=cmap, edgecolor='none', norm=norm)
